...
Foydalanuvchiga yordam berasan: kunlik rejalashtirish, ovqat, pul tejash, 
ob-havo, life hacklar, motivatsiya, sotuvchilik va biznes.
Har doim qisqa, aniq va amaliy javob ber. O'zbek tilida gapir."""
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompts = {
        "🌤 Ob-havo": f"{YOUR_CITY}da bugun ob-havo qanday ({datetime.now().strftime('%B')})? Qisqa ayt.",
        "👔 Kiyinish": f"{YOUR_CITY}da bugun qanday kiyinish kerak? Erkak va ayol uchun.",
        "🥗 Ovqat": f"Bugun {'nonushta' if datetime.now().hour < 11 else 'tushlik' if datetime.now().hour < 15 else 'kechki ovqat'} uchun 3 ta sog'lom tavsiya ber.",
        "💰 Pul tejash": "O'zbekistonda pul tejash bo'yicha 3 ta amaliy life hack ber.",
        "📋 Vazifalar": f"Mening vazifalarim: {', '.join(get_user(user_id)['tasks']) or 'yo\'q'}. Qanday bajaraman?",
        "🎯 Maqsadlar": f"Mening maqsadlarim: {', '.join(get_user(user_id)['goals']) or 'yo\'q'}. Bugun nima qilaman?",
        "💡 Life hack": "Bugun uchun bitta foydali life hack ber.",
        "🧠 Muammo hal": "Bugun uchun bitta amaliy muammo va yechimini ber. Growth mindset asosida.",
        "📊 Hisobot": f"Maqsadlar: {len(get_user(user_id)['goals'])} ta, Vazifalar: {len(get_user(user_id)['tasks'])} ta. Qisqa motivatsion xabar yoz.",
        "💬 Suhbat": None
    }
